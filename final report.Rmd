...
The objective of the study is to evaluate the association between \textit{ Serum Prostate-specific antigen level (mg/ml) \textbf{(PSA)}} and other clinical measurements in men with advanced prostate cancer
...
\begin{itemize}
  \item \textbf{VOL} The plot indicates the distribution may not have a normal distribution and shows two potential outliers should be checked.
...
Figure \ref{fig:dist.cat} shows the distribution of the of SCORE and SEM factors
...
Figure \ref{fig:fig.hist.cov} indicates the \textbf{VOL}, \textbf{HYP} amd \textbf{CAP} my not have normal distributions.  

```{r fig.hist.cov, echo=FALSE, message=FALSE, warning=TRUE, fig.cap="Histogram of cancerv, capsular and hyperplasia"}
hist.cov.theme <-theme(plot.title = element_text(color="blue",
        size=10,
        face="bold",
        hjust = 0.5),
        axis.text.x = element_blank(),
...
```{r, tab.stat.summary, message=FALSE, warning=FALSE, echo=FALSE}
...
\ref{tab.lm.summary} indicates \textbf{VOL}, \textbf{$sCORE_{score=8}$} and \textbf{$SEM_{presence}$} are all statistically significant.
