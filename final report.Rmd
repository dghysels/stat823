...
Determine whether compound XYZ reduces human leukocyte antigen (HLA) alloreactivity as measured by calculated panel reactive antibodies (cPRA) at week 16 compared to baseline (week 0) measures.
...
This is how you include R code within RMarkdown:
````
```{r, message=FALSE}`r ''`
```
````
...
The correlation between psa and weight and psa and age are considered tool low to consider these variables these variables as potential covariates. 

```{r distribution1, echo=FALSE, include=TRUE, fig.cap="Distribution of continuous variables",fig.pos="H", fig.align='center', out.width="0.6\\linewidth"}
p1 <- ggplot(data2, aes(y=cancerv,x=1)) + 
  geom_boxplot() + geom_jitter(position=position_jitter(0.05))
...
Figure \ref{fig:distribution1} plots the distribution for all continuous variables being considered as potential covariates.  The plots show the distribution is noticibly skewed in each case

\subsection{visualize factors}

Figure \ref{fig:distribution2} shows the distribution of the of Score and Seminal factors
```{r distribution2, echo=FALSE, include=TRUE, fig.cap="distribution of factor variables",fig.pos="H", fig.align='center', out.width="0.6\\linewidth"}
par(mfrow = c(1,2))
...
```{r distribution3, echo=FALSE, message=FALSE, warning=TRUE, fig.cap="Histogram of cancerv, capsular and hyperplasia"}
#par(mfrow=c(2,2))
p1 <- ggplot(data2, aes(x=cancerv)) + 
  geom_histogram(binwidth = 2)
p2 <- ggplot(data2, aes(x=capsular)) + 
  geom_histogram(binwidth = 1)
p3 <- ggplot(data2, aes(x=hyperplasia)) + 
  geom_histogram(binwidth = 0.5)
grid.arrange(ncol = 2, p1,p2,p3)
```

Figure \ref{fig:distribution3} shows the skewness of the three potential covariates, indicating the data may not be normally distributed and problems with non constant variance may exist

\section{Compare Covariates}

```{r echo=FALSE}
b <- regsubsets(psa ~ cancerv+hyperplasia+seminal+capsular+score, data = data2)
rs <- summary(b)
...
```{r distribution4, echo=FALSE, fig.cap="Visualize cancer volume distribution", message=FALSE,fig.pos="H", fig.align='center', out.width="0.6\\linewidth"}
#lm.capsular <- lm(data = data2, psa ~ capsular)
lm.cancerv <- lm(data = data2, psa ~ cancerv)
lm.seminal <- lm(data = data2, psa ~ seminal)
lm.score <- lm(data = data2, psa ~ score)
...
Figure \ref{fig:distribution4} Shows very clear evidence of distribution problems.  The non constant variance test and shapiro-wilks test both indicate the data violates the assumption of constant variance and normal distribution


```{r tab3, results="asis",echo=FALSE, include=TRUE, message=TRUE}
ncvp <-ncvTest(lm.cancerv.t)$p
swp <- shapiro.test(residuals(lm.cancerv.t))$p.value
t<- data.frame(a=c(swp),b=c(ncvp))
knitr::kable(t,col.names =c("Shapiro-wilks p-value","ncv test p-value"),label="tab:tab3",caption = "Shapiro-wilks and ncv test after log log transformation")
```

The non-constant variance test and shapiro wilks test in table \ref{tab1} both indicate a problem with the distribution of the residuals

The data shows problems with normal distribution which must be corrected if the data is to be used for inference

\subsection{log log transformation}

A log transformation of both psa and cancerv was found to provide a normal distribution of the residuals and also removed the problems with homoscedacity.
...
```{r distribution5, echo=FALSE, include=TRUE, fig.cap="cancer volume distribution after a log log transformation",fig.pos="H", fig.align='center', out.width="0.6\\linewidth"}
lm.cancerv.t <- lm(log(psa) ~ log(cancerv), data = data2)
...
plot(fitted(lm.cancerv.t),residuals(lm.cancerv.t), 
     xlab="residuals",
     ylab="fitted",
     main = "Log(Cancer Vol.)\n fitted vs residuals")
abline(h = 0)
