...
data2$hyperplasia.log <- log(data2$hyperplasia + 0.0001)
data2$capsular.log <- log(data2$capsular + 0.0001)
...
par(mfrow=c(1,2),mar=c(5,4,4,3))
plot(y=log(data2$psa),x=data2$hyperplasia.log, 
     xlab="log(PSA)", 
     ylab="log(HYP)", 
     main="log(HYP) \n Regression Fit")
...
par(mfrow=c(1,2),mar=c(5,4,4,3))
plot(y=log(data2$psa),x=data2$capsular.log, 
     xlab="log(PSA)", 
     ylab="log(CAP)", 
     main="log(CAP) \n Regression Fit")
...
b <- regsubsets(log(psa) ~ log(cancerv)+seminal+hyperplasia.log+capsular.log+log(weight)+log(age), data = data2)
regsub <- summary(b)
...
colnames(regsubdf) <- c("Intercept","VOL","SEM","HYP","CAP","WT","AGE")
...
```{r ref.label=knitr::all_labels(), eval=FALSE}
  ...
  results <- data.frame(pred, actual,"pct error"=paste("%",((pred-actual)/actual)*100))
  results
