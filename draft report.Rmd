...
#data2$weight[data2$weight > 450] <- data2$weight[data2$weight > 450]/10
...
lm.hyperplasia.log <- lm(data = data2, log(psa) ~ log(hyperplasia))
lm.capsular <- lm(data = data2, psa ~ capsular)
lm.capsular.log <- lm(data = data2, log(psa) ~ log(capsular))
lm.weight <- lm(data = data2, psa ~ weight)
lm.weight.log <- lm(data = data2, log(psa) ~ log(weight))
lm.age <- lm(data = data2, psa ~ age)
lm.age.log <- lm(data = data2, log(psa) ~ log(age))
...
summ.hyperplasia.log.all <- c(summ.hyperplasia.log$coefficients[c(2,4,6,8)],
                              summ.hyperplasia.log$adj.r.squared,summ.hyperplasia.log$fstatistic[1])
summ.capsular.all <- c(summ.capsular$coefficients[c(2,4,6,8)],summ.capsular$adj.r.squared,summ.capsular$fstatistic[1])
summ.capsular.log.all <- c(summ.capsular.log$coefficients[c(2,4,6,8)],summ.capsular.log$adj.r.squared,summ.capsular.log$fstatistic[1])
summ.age.all <- c(summ.age$coefficients[c(2,4,6,8)],summ.age$adj.r.squared,summ.age$fstatistic[1])
summ.age.log.all <- c(summ.age.log$coefficients[c(2,4,6,8)],summ.age.log$adj.r.squared,summ.age.log$fstatistic[1])
summ.weight.all <- c(summ.weight$coefficients[c(2,4,6,8)],summ.weight$adj.r.squared,summ.weight$fstatistic[1])
summ.weight.log.all <- c(summ.weight.log$coefficients[c(2,4,6,8)],summ.weight.log$adj.r.squared,summ.weight.log$fstatistic[1])
...
kable(summarydf,label = "tab.fitted.summary", caption = "Goodness-of-fit", digits = 3)
...
plot(y=data2$psa,x=data2$age, 
     ...
     ylab="AGE")
box(col="blue")
mtext("AGE Fitted", side=3, line=0, cex=1, col="blue")
...
plot(fitted(lm.age),residuals(lm.age), 
     ...
     ylab="fitted")
box(col="blue")
mtext("AGE Fitted vs Residuals", side=3, line=0, cex=1, col="blue")
abline(h = 0,col="forestgreen")
...
plot(y=data2$psa,x=data2$hyperplasia, 
     ...
     ylab="HYP")
box(col="blue")
mtext("HYP Fitted", side=3, line=0, cex=1, col="blue")
...
plot(fitted(lm.hyperplasia),residuals(lm.hyperplasia), 
     ...
     ylab="fitted")
box(col="blue")
mtext("HYP Fitted vs Residuals", side=3, line=0, cex=1, col="blue")
abline(h = 0,col="forestgreen")
...
```{r fig.log.WT.fitted, include=TRUE, fig.cap="WT Fitted AFter Log Transformation", fig.pos="H", fig.align='center', out.width="0.8\\linewidth"}
...
plot(fitted(lm.weight.log),residuals(lm.weight.log), 
     xlab="residuals",
     ylab="fitted",
     main = "Log(WT)\n fitted vs residuals")
abline(h = 0)
...
```{r fig.log.HYP.fitted, include=TRUE, fig.cap="HYP Fitted AFter Log Transformation", fig.pos="H", fig.align='center', out.width="0.8\\linewidth"}

par(mfrow=c(1,2),mar=c(5,4,4,3))
plot(y=log(data2$psa),x=log(data2$hyperplasia), 
     xlab="log(PSA)", 
     ylab="log(HYP)", 
     main="log(HYP) \n Regression Fit")
abline(lm.hyperplasia.log)

plot(fitted(lm.hyperplasia.log),residuals(lm.hyperplasia.log), 
     xlab="residuals",
     ylab="fitted",
     main = "Log(HYP)\n fitted vs residuals")
abline(h = 0)
...
```{r fig.log.CAP.fitted, include=TRUE, fig.cap="HYP Fitted AFter Log Transformation", fig.pos="H", fig.align='center', out.width="0.8\\linewidth"}

par(mfrow=c(1,2),mar=c(5,4,4,3))
plot(y=log(data2$psa),x=log(data2$capsular), 
     xlab="log(PSA)", 
     ylab="log(CAP)", 
     main="log(CAP) \n Regression Fit")
abline(lm.capsular.log)

plot(fitted(lm.capsular.log),residuals(lm.capsular.log), 
     xlab="residuals",
     ylab="fitted",
     main = "Log(CAP)\n fitted vs residuals")
abline(h = 0)
...
```{r}
summarydf5 <- data.frame("VOL"=summ.cancerv.all,
                         "log(VOL)"=summ.cancerv.log.all,
                        "AGE"=summ.age.all,
                        "log(AGE)"=summ.age.log.all,
                        "WT"=summ.weight.all,
                        "log(WT)"=summ.weight.log.all)
rownames(summarydf5) <- c("beta1","std Err","t-value","p-value","adj. R2","f-statistic")

kable(summarydf5,label = "tab.fitted.summary", caption = "Goodness-of-fit", digits = 5)
```
...
b <- regsubsets(log(psa) ~ log(cancerv)+seminal+log(weight), data = data2)
regsub <- summary(b)
...
colnames(regsubdf) <- c("Intercept","VOL","SEM","WT")
...
for(i in 1:nrow(regsubdf)){
  r <- regsubdf[i,]
  lbl <- vector()
  if ("VOL" %in% colnames(regsubdf) && regsubdf[i,"VOL"] == TRUE){
    lbl <-append(lbl, "VOL")
  }
  if ("cap" %in% colnames(regsubdf) && regsubdf[i,"CAP"] == TRUE){
    lbl <- append(lbl, "CAP")
  }
  if ("SEM" %in% colnames(regsubdf) && regsubdf[i,"SEM"] == TRUE){
    lbl <- append(lbl, "SEM")
  }
  if ("SCORE7" %in% colnames(regsubdf) && regsubdf[i,"SCORE7"] == TRUE){
    lbl <- append(lbl, "SCORE(7)")
  }
  if ("SCORE8" %in% colnames(regsubdf) && regsubdf[i,"SCORE8"] == TRUE){
    lbl <- append(lbl, "SCORE(8)")
  }
  if ("AGE" %in% colnames(regsubdf) && regsubdf[i,"AGE"] == TRUE){
    lbl <- append(lbl, "AGE")
  }
  if ("WT" %in% colnames(regsubdf) && regsubdf[i,"WT"] == TRUE){
    lbl <- append(lbl, "WT")
  }
   if ("HYP" %in% colnames(regsubdf) && regsubdf[i,"HYP"] == TRUE){
    lbl <- append(lbl, "HYP")
  }
  if ("CAP" %in% colnames(regsubdf) && regsubdf[i,"CAP"] == TRUE){
    lbl <- append(lbl, "CAP")
...
regsubdf <- data.frame(rowlabels,"RSS" = regsub$rss, 
                       ...
                       "cp"=regsub$cp, 
                       "bic"=regsub$bic)
#regsubdf[,2:5] <- round(regsubdf[,2:5],4)
kable(regsubdf, digits = 3
...
lm.multi <- lm(data=data2, log(psa) ~ log(cancerv) + log(weight) + seminal)
