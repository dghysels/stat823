...
The summary statistics for the Prostate Cancer data are presented in Table \ref{tab:tab.summary} do not indicate any initial concerns regarding potential outliers or the spread of the data.
...
A fitted model for potential covariates allows evaluation of the adjusted $R^{2}$ fit and the p-value for each.  The models \ref{tab.fitted.summary} indicate a weak linear relationship between the dependent variable \textbf{PSA} and the independent variables \textbf{VOL}, \textbf{SCORE}, \textbf{SEM} and \textbf{CAP} based on the adjusted $R^{2}$.  The p-value < 0.05 indicates the null hypothesis; a significant relationship exists between these same variables.  \textbf{HYP}, \textbf{AGE} and \textbf{WT} have a p-value > 0.05; an alternate hypothesis must be accepted.
...
The non-constant variance test and Shapiro-Wilks is used to determine if the data violate the assumption of constant variance and normal distribution.  The Shapiro-Wilks p-value is <0.05 for all covariates, indicate the null hypothesis of a normal distribution of residuals most be rejected.  The non-constant variance p-values indicate non-constant variance hypothesis must be rejected for \textbf{VOL} and \textbf{CAP}.
...
The regression assumptions require a constant variance and normal distribution of residual errors, which the data does not achieve. A log transformation of dependent variable \textbf{PSA} and the independent variables \textbf{VOL}, \textbf{CAP}, \textbf{HYP}, \textbf{WT} generates an improved distribution of the residuals and non-constant variance for each of these variables.  The improvement is particularly pronounced for \textbf{WT} and \textbf{VOL}
...
The goodness-of-fit values as shown in in table \ref{tab.fitted.summary2} indicates the log transformation improved the $Adj.R^{2}$ and the p-values for \textbf{VOL} and \textbf{WT} indicating these variables now have a stronger linear relationship to \textbf{PSA}.  A comparison of the adjusted p-values before the transformation shows \textbf{WT} is not considered significant.  The p-value after the transformation now supports the null hypothesis \textbf{WT} has a significant relationship with \textbf{PSA} 
...
The residuals plot indicates the residuals for the regression model are normally distributed and the variance is constant across the x-axis.  The observations are verified using the shapiro-wilks test and the non-constant variance test.  The output from the Shapiro-Wilks test indicates the null hypothesis of a normal distribution of the residuals must be accepted.  Similarly, table \ref{tab.ncv.test} indicates the null hypothesis of constant variance must also be accepted.  The Variance Inflation Factor in table \ref{tab.vif} also indicates all values are less 5 and the model does not exhibhit problems with collinearity  
...
```{r tab.ncv.test, results='asis', include=TRUE, fig.cap="non-constant variance (Breusch-Pagan)test",fig.pos="H", fig.align='center', out.width="0.6\\linewidth"}
...
xtable(ncv.out,
       label = "tab.ncv.test",
       caption = " Non-Constant Variance Test")
```
...
xtable(vifdf,label = "tab.vif",
       caption = "Variance Inflation Factors test for Multicollinearity")
```
  

To demonstrate the performance of the model, a random sample of 85% of the data is used to train the model and 15% held to test the model.  Table \ref{actual.pred} provides a summary of the predicted response vs. the actual PSA value.  


```{r actual.pred, results='asis'}
...
#output the results in a data frame
  ...
  xtable(results, label = "actual.pred", caption = "Test The Model - Actual vs. Predicted")
