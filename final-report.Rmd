...
To support linear regression, the errors are assumed to be normally distributed, the residuals of the dependent variable are assumed to be evenly distributed (constant variance) across the range of possible independent (x axis) values.  Because the model will use multiple covariates, the data is also assumed the outcomes are not influenced by multicollinearity \citep{wiley}.  

If the data does not support the assumptions, a non-linear transformation of one or more variables may be used \citep{wiley}.  The use of a common transformation such as the use of a natural logarithm requires any observations with a value of zero to be first transformed.  Given the values of zero for \textbf{CAP} and \textbf{HYP}, a linear transformation is applied to all values.  A value of 0.0001 has been added to each observation.

The final model is tested to verify these key assumptions hold true.  
...
xtable(summarydf,label = "tab.fitted.summary", caption = "Goodness-of-fit")
...
#non-constant variance for the untransformed 
#fitted models
...
The goodness-of-fit values as shown in in table \ref{tab.fitted.summary2} indicates the log transformation also improved the $Adj.R^{2}$ and the p-values for \textbf{VOL} and \textbf{WT}.  A comparison of the adjusted $Adj.R^{2}$ before the transformation \textbf{WT} was not considered significant and after the transformation the p-value clearly indicates \textbf{WT} has a signicant relationship with \textbf{PSA} 
...
#----- create a table of the transformed regression summaries
# with the before transformation and after transformation
...
#------ use regsubsets to create to determine
#the best model
...
#----display the output of the multiple regression
...
# and adjusted R2 for the multiple regressoin
# model
...
#----get the most influential HAT values for the
...
#cooks distance and output as a table
...
#---- Test the model by removing the two most influential
#observations 
...
The plot \ref{fig.resid.vs.fitted} indicates the residuals for the regression model are normally distributed and the variance is constant across the x-axis.  The observations are verified using the shapiro-wilks test and the non-constant variance test.  The output from the Shapiro-Wilks test indicates the null hypothesis of a normal distribution of the residuals must be accepted.  Similarly, table \ref{tab.ncv.test} indicates the null hypothesis of constant variance must also be accepted.  The Variance Inflation Factor in table \ref{tab.vif} also indicates all values are less 5 and the model does not exhibhit problems with collinearity  
