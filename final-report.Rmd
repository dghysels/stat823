...
---
  ...
  \pagestyle{fancy}
  \fancyhead[L]{\textbf{Don, Madhumithra, Melanie}}
  \fancyhead[C]{}
   ...
   \begin{center}
       ...
       \textbf{\Large STAT 823: Spring Class Project, 2021} 
        ...
        \vspace{0.5cm}
         ...
         \textbf{\large Don Ghysels, Madhumithra Subramanian Karthikes, Melanie Valdez}
...
\section{Abstract}
Healthcare providers require accurate markers to understand the disease progression for patients with prostate cancer.  Monitoring the protein prostate-specific antigen (PSA) is an accepted method of tracking the prostate health of men, but its value has fallen over the past three decades.  However, the data collected while studying cancer patients may still be considered a valuable tool for students learning to evaluate clinical data.  The goal of this investigation is to understand the relationship between PSA and 7 other clinical measures. 

\section{Introduction}

Tracking the level of the protein prostate-specific antigen (PSA) in men diagnosed with prostate cancer is considered an FDA approved method of monitoring the disease (\citet{cit.nat.cancer.inst}). While testing PSA levels is still considered a valuable tool for monitoring, its use as a diagnostic tool is decreasing.  A strong relationship was found between PSA and the clinical stage of prostate cancer (\citet{stamey1989}), but a later study by the same author found this correlation to be slowly decreasing (\citet{stamey2004}) due to changes in the way the disease is treated.  While the 2004 study noted PSA is strongly correlated with the  weight of the prostate, the enlargement may likely be due to benign causes.  The 2004 study concludes a new marker is needed to replace PSA.  While the value of PSA as a prostate cancer indicator is decreasing, the 1989 study has been widely used and is referenced in several other studies until at least 2011 (\citet{netto}), and continues to be a useful tool for teaching data analysis (\citet{love})  The objective of this study is to evaluate data from the 1989 study to look for the association between \textbf{PSA} and 7 clinical measurements in men with advanced prostate cancer.  The measurements include \textbf{estimate of prostate cancer volume}, \textbf{prostate weight}, \textbf{age of patient}, \textbf{amount of benign prostatic hyperplasia}, \textbf{degree of capsular penetration}, \textbf{seminal vesical Invasion}, and the \textbf{Gleason Score}.

\section{Materials and Methods}

The dataset for this analysis was originally published by \citet{stamey1989} and then adapted by the textbook \emph{"Elements of Statistical Learning"} \citet{hastie}.  This text from Springer is credited in the \href{https://courseware.ku.edu/bbcswebdav/pid-9064398-dt-content-rid-52612040_1/courses/4212-58856/Class_Project%282%29.pdf}{stat823 course} project materials as the source of the adapted data.  A summary of the dependent and independent variables available is presented in Table \ref{table:model.variables}
...
Gleason Score & na & SCORE & Categorical \\ [1ex] % [1ex] adds vertical space
...
\textbf{PSA} is found to have significant relationship with the three covariates \textbf{WT}, \textbf{SEM} and \textbf{VOL} using multiple linear regression. 

\section{Statistical Analysis}

The data are analyzed using multiple linear regression using the R programming language `r R.version.string` [R](https://www.r-project.org/) \citep{r2021}. The data contain 97 observations and do not appear to include any missing values.  Hyperplasia and Capsular contain observations with a value of zero which are assumed to be correctly recorded observations.  
 
The adjusted $R^{2}$ is used to check the strength of the linear relationship between the dependent and independent variables. The relationship between the dependent and independent variables is checked by using p-values and hypothesis tests.  All inferences are conducted using $\alpha = 0.05$ unless stated otherwise. 
...
To support linear regression and inference, several key assumptions must be verified (\citep{wiley}).  First, a linear relationship must exist between the independent and dependent variables. Also, the errors are assumed to be normally distributed and the residuals of the dependent variable are assumed to be evenly distributed (constant variance) across the range of possible independent (x axis) values.  Because the model will use multiple covariates, the model assumes the data are not influenced by multicollinearity.  

If the data do not support the assumptions, a non-linear transformation of one or more variables may be used \citep{wiley}.  The use of a common transformation such as a natural logarithm requires any observations with a value of zero to be first modified using a linear transformation.  Given the values of zero for \textbf{CAP} and \textbf{HYP}, a value of 0.0001 has been added to each of these two variables for all observations.  No other variables required a linear transformation prior to the log transformation.  

The final model is tested to verify the assumptions required for linear regression.  The R ncvtest function from the \href{https://www.rdocumentation.org/packages/car/versions/3.0-10/topics/ncvTest}{car} package is used to verify constant variance and  the Shapiro-Wilk's test from \href{https://stat.ethz.ch/R-manual/R-devel/library/stats/html/shapiro.test.html}{stats package} to verify a normal distribution of the residuals.
...
dataraw <- read_xlsx("./pcancer.xlsx")
...
#The data contains a single value of 450g.  
#This has been determined 
#to be an error in the dataset. 
#Correct the value before analysis
data2$weight[32] <-44.9

#this function is used to format decimal 
#values stored in a data frame
...
The summary statistics for the Prostate Cancer data presented in Table \ref{tab:tab.summary} do not indicate any initial concerns regarding potential outliers or the spread of the data.
...
colnames(summ.table) <- c("Variable",  
                          "Obs. Count",  
                          "Mean",  
                          "Median", 
                          "SD",  
...
Figure \ref{fig:fig.dist.VOL} through and Figure \ref{fig:fig.dist.AGE} summarize the data distributions for the 5 continuous variables.  The distributions will help identify potential outliers and help determine if the data have a normal distribution or if the data exhibit skewness
...
\begin{itemize}
  ...
  \item \textbf{CAP} data clearly contain a group of observations with a value of zero. These values are assumed to be valid observations and not missing values.
  \item \textbf{HYP} data clearly contain a group of observations with a value of zero. These values are assumed to be valid observations and not missing values.
  \item \textbf{WT}  contains observations that should be examined as potential outliers.
  \item \textbf{AGE} data appear to have a normal distribution and do not show outliers.
\end{itemize}
...
Figure \ref{fig:fig.dist.cat} shows the distribution of the of SCORE and SEM factors.  The frequencies shown in table \ref{tab.sem.freq} and and table \ref{tab.score.freq} indicate a much smaller frequency for \textbf{$SEM_{presence}$} than for \textbf{$SEM_{absence}$}

```{r tab.sem.freq, results='asis' }
#-------- Frequency table for SEM 
xtable(tab.sem$output.table, 
       caption = "SEM frequency plot", 
       label="tab.sem.freq")
```
...
A fitted model for potential covariates allows evaluation of the adjusted $R^{2}$ fit and the p-value for each.  The adjusted $R^{2}$ presented in table \ref{tab.fitted.summary} indicates a weak linear relationship between the dependent variable \textbf{PSA} and the independent variables \textbf{VOL}, \textbf{SCORE}, \textbf{SEM} and \textbf{CAP}.  The p-value < 0.05 indicates the null hypothesis; a significant relationship exists between these same variables.  \textbf{HYP}, \textbf{AGE} and \textbf{WT} have a p-value > 0.05; an alternate hypothesis must be accepted.
...
The non-constant variance test and Shapiro-Wilk's is used to determine if the data violate the assumption of constant variance and normal distribution.  The Shapiro-Wilk's p-value is <0.05 for all covariates, indicate the null hypothesis of a normal distribution of residuals must be rejected.  The non-constant variance p-values indicate the non-constant variance hypothesis must be rejected for \textbf{VOL} and \textbf{CAP}.
...
colnames(cancerv.normdf) <- c("NCV","Shapiro-Wilk's")
rownames(cancerv.normdf) <- c("VOL",
                              "WT",
                              "AGE",
                              "CAP",
                              "HYP")
...
kable(cancerv.normdf,label="tab:tab.ncv",
       table.placement="H",
       caption = "Shapiro-Wilk's and Non-Constant Variance (NCV) test")
```

\subsection{Transformations}

The regression assumptions require a constant variance and normal distribution of residual errors, which the data do not achieve. A log transformation of the dependent variable \textbf{PSA} and the independent variables \textbf{VOL}, \textbf{CAP}, \textbf{HYP}, and  \textbf{WT} generates an improved distribution of the residuals and non-constant variance for each of these variables.  The improvement is particularly pronounced for \textbf{WT} and \textbf{VOL}
...
The goodness-of-fit values as shown in in table \ref{tab.fitted.summary2} demonstrates the log transformation improved the $Adj.R^{2}$ and the p-values for \textbf{VOL} and \textbf{WT} indicating these variables now have a stronger linear relationship to \textbf{PSA}.  A comparison of the adjusted p-values before the transformation shows \textbf{WT} is not considered significant.  The p-value after the transformation now supports the null hypothesis \textbf{WT} has a significant relationship with \textbf{PSA} 

The Shapiro-Wilk's test using the transformed data shown in table \ref{tab.log.ncv} indicates all the null hypotheses of normal distribution of the residuals must be accepted.  The non-constant variance test indicates the null hypothesis should be accepted for all except \textbf{CAP} and \textbf{HYP}  
...
xtable(summarydf2,label = "tab.fitted.summary2",
       caption = "Goodness-of-Fit after Log Transformation", 
       table.placement="H")
```
...
#set some column and row names
colnames(cancerv.normdf) <- c("Non-Constant Variance","Shapiro-Wilk's")
rownames(cancerv.normdf) <- c("VOL",
                              "WT",
                              "AGE",
                              "CAP",
...
xtable(cancerv.normdf,
      label="tab.log.ncv",
       table.placement="H",
       caption = "Shapiro-Wilk's and NCV Test After Log Transformation")
```
...
xt <- xtable(regsubdf, digits = 3
       , label = "tab.covariate.sel"
       ,table.placement="H")
print(xt, size="\\fontsize{10pt}{12pt}\\selectfont")
...
Final model selection is aided by the regsubsets function available in the leaps R package.  The automatic covariate selection output indicates the model with the lowest BIC, cp and the highest $R^{2}$ is a complex five covariate model.  Based on the issues already discovered with \textbf{HYP} residuals, models that include this variable are rejected.  A four covariate model is now best, but the improvements in $R^{2}$ and BIC are very small when compared to the three covariate model.  Based on the small improvements and concerns over complexity and potential overfitting, the final model uses the following three covariates: 
...
```{r tab.lm.summary,warning=FALSE, message=FALSE, results='asis'}
#----display the summary of the final model
#model as a formatted table
tbl_regression(lm.multi)
#xtable(summary(lm.multi))
```
The adjusted $R^{2}$ value in table \ref{tab.summ.stat} indicates `r (summary(lm.multi)$adj.r.squared)*100` percent of the variability in the data can be explained by the model.  This $R^{2}$ represents a moderate linear relationship between the dependent and independent variables.

The response formula is as follows
$log(PSA)$ = $\beta_{0} + \beta_{1}log(VOL) + \beta_{2}Log(weight) + \beta_{3}SEM_{presence}$ 
...
The largest Hat values are used to determine the observations that may have the greatest influence on the final model.  The hat values shown in table \ref{tab.largest.hat} Observations do not show any values that would be expected to have a large influence on the regression slope ($\beta$).  Similarly, table \ref{tab.cooksd} of the largest Cook's distance values does not indicate any observations would be expected to have a large influence

As verification, table \ref{tab.model.comp} compares \textbf{model 2} with the two most influential values remove is compared with the full \textbf{model 1}. Removing the two most influential observations has only a small effect on the $\beta$ slope values and the $Adj.R^{2}$ 
...
largest.hat <- data.frame(head(sort(lm.multi.influence$hat,
                                    decreasing=TRUE),6))
...
cooksd <- data.frame(head(sort(cooks.distance(lm.multi),
                               decreasing=TRUE),6))
colnames(largest.hat) <- c("Hat Influence")
xtable(largest.hat, 
      label="tab.cooksd", 
...
largest.cooksd <- head(sort(cooks.distance(lm.multi),
                            decreasing=TRUE),2)
...
The residuals vs. fitted values plot indicates the residuals for the regression model are normally distributed and the variance is constant across the x-axis.  The observations are verified using the Shapiro-Wilk's test and the non-constant variance test.  The output from the Shapiro-Wilk's test indicates the null hypothesis of a normal distribution of the residuals must be accepted.  Similarly, table \ref{tab.ncv.test} indicates the null hypothesis of constant variance must also be accepted.  The Variance Inflation Factor in table \ref{tab.vif} also indicates all values are less 5 and the model does not exhibit problems with multicollinearity  
...
```{r fig.shapiro, include=TRUE, fig.cap="Shapiro-Wilk's Test of Residuals",fig.pos="H", fig.align='center', out.width="0.6\\linewidth"}
#plot the shapio-wilks p-value and qqnorm plot
shapiro.qqnorm(residuals(lm.multi), cex = 2)
```

```{r tab.ncv.test, include=TRUE, results='asis', fig.cap="non-constant variance (Breusch-Pagan)test",fig.pos="H", fig.align='center', out.width="0.6\\linewidth"}
...
The model performance is tested by randomly sampling 85% of the data to create training data.  The remaining 15% of the data are used to test the model after training.  Table \ref{tab.pred.actual} prvides a summary of the predicted response vs. the actual PSA value

```{r tab.pred.actual, results='asis'}
#test the model by creating test and training data sets
  set.seed(10)
  sp <- sample(2, nrow(cars), replace = TRUE, prob = c(0.85, 0.15))
  data2.train <- data2[sp==1,]
  data2.test <- data2[sp==2,]

#create a new model using the training data set
  lm.multi3 <- lm(data=data2.train, log(psa) ~ log(cancerv) +
                                                log(weight) + 
                                                seminal)
  
#use the test data set to make predictions about the response
  pred <- exp(predict(lm.multi3, data2.test, type='response'))
#get the actual response values from the data 
  actual <- data2.test$psa
#compute the error between predicted and actual
  err <- (pred-actual)/actual
#output the results in a data frame
  results <- data.frame(pred, actual,
                        "pct error"=paste("%",round(err*100,1),sep=""))
  xtable(results, label = "tab.pred.actual", 
         caption="Test the Model - Predicted vs. Actual results")
  
```
RESULTS:

In the primary analysis, the significant clinical measurements that has linear relationship with the PSA was found out. Initially, the correlation coefficients between the PSA and other continuous model covariates were studied. Only CAP and VOL showed significant relationship with PSA. Further the data distribution for the continuous variables were analyzed using   distribution plots and summary statistics. AGE had normal distribution and VOL had skewed distribution. WT and VOL were found to have potential outliers. In the initial analysis of non-constant variance and the normality assumption, only AGE, WT and HYP satisfied homoscedasticity and none of the continuous variables satisfied the normality assumption.

The log transformations performed on both the dependent variable PSA and other independent variables such as VOL and WT resulted in a significant improvement in distribution and non-constant variance along with a stronger linear relationship with PSA which was evident from the goodness-of-fit test.

Using regsubsets function, the model with 5 variables VOL, SEM, SCORE (8), WT and HYP had the lowest BIC, $C_p$ and highest $R^2$. But since HYP had issues, we considered model without HYP. As there was no significant improvement in $R^2$ and BIC values between three and four covariate models, the three-covariate model VOL, SEM, WT was selected with slight tuning considering only seminal presence true values.

CONCLUSIONS AND DISCUSSIONS:

From the summary and goodness of fit tables of the final model, all the chosen final parameters log (VOL), log (WT) and $SEM_{presence}$ of the multiple linear regression showed significant relationship with log(PSA) with p-values less than 0.05. The final model has a variability of 62.34387%.

The response formula for the final model was $log(PSA) = \beta_0 + \beta_1log(V OL) + \beta_2log(weight) + \beta_3SEM_{presence}$, where $\beta_0 = -0.77074, \beta_1 = 0.52512, \beta_2  = 0.66037 and \beta_3 = 0.66596$.  The final model was tested for all the assumptions – influential points, normality, non-constant variance, outliers and variance inflation factor. There were six influential points, but the influence was not significant. However, it was found that removing two most influential points from the model has insignificant effect on adjusted $R^2$ and slopes. The final model also satisfied the normality assumption($p-value=0.9873$) and homoscedasticity assumption($p-value=0.63$). Also, all the variance inflation factors were less than 5. 13 outliers were found for the final model.

The final model designed with three covariates is the best multiple regression model for the given data that satisfies all the assumptions.Our study shows that cancer volume, weight and seminal presence can predict the prostate specific antigen with appropriate log transformation for VOL, WT and PSA. The chosen model was able to explain 62% of variation in the prostate specific antigen in the prostate cancer patients.
\newpage
