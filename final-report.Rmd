...
\textbf{PSA} is found to have significant relationship with the three covariates \textbf{WT}, \textbf{SEM} and \textbf{VOL} using mutliple linear regression. 
...
To support linear regression and inference several key assumptions must be verified (\citep{wiley}).  First, a linear relationship must exist between the independent and dependent variables, the errors are assumed to be normally distributed, the residuals of the dependent variable are assumed to be evenly distributed (constant variance) across the range of possible independent (x axis) values.  Because the model will use multiple covariates, the model assumes the data is not influenced by multicollinearity.  
